...
// test data.
func genRandomSig() (*chainhash.Hash, *ecdsa.Signature, *eunoec.PublicKey, error) {
	privKey, err := eunoec.NewPrivateKey()
	if err != nil {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	key1Copy, _ := eunoec.ParsePubKey(key1.SerializeCompressed())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		keyCopy, err := eunoec.ParsePubKey(key.SerializeCompressed())
		if err != nil {
	...
	keyNewCopy, _ := eunoec.ParsePubKey(keyNew.SerializeCompressed())
