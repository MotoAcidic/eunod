...
// newAddressPubKey returns a new eunoutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) eunoutil.Address {
	addr, err := eunoutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new eunoutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) eunoutil.Address {
	addr, err := eunoutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new eunoutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) eunoutil.Address {
	addr, err := eunoutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the eunoutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the eunoutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := eunoutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := eunoutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := eunoutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := eunoutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := eunoutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*eunoutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*eunoutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*eunoutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*eunoutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*eunoutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
