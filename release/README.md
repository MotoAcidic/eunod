# `eunod`'s Reproducible Build System

This package contains the build script that the `eunod` project uses in order to
...
and `cmd/eunoctl/version.go`.
For example (taken from [f3ec130](https://github.com/MotoAcidic/eunod/commit/f3ec13030e4e828869954472cbc51ac36bee5c1d)):
```diff
diff --git a/cmd/eunoctl/version.go b/cmd/eunoctl/version.go
index 2195175c71..f65cacef7e 100644
--- a/cmd/eunoctl/version.go
+++ b/cmd/eunoctl/version.go
...
1. `git clone https://github.com/MotoAcidic/eunod.git`
2. `cd eunod`
3. `./release/release.sh <TAG> # <TAG> is the name of the next release/tag`

This will then create a directory of the form `eunod-<TAG>` containing archives
...
Now that the directory `eunod-<TAG>` is created, the manifest file needs to be
...
which roughly follow the format of [previous release notes](https://github.com/MotoAcidic/eunod/releases/tag/v0.20.1-beta).
...
When following the GitHub documentation, include every file in the `eunod-<TAG>`
directory.
...
5. Ensure `go` is installed, matching the same version as noted in the release
   notes. 
6. Obtain a copy of `eunod`'s source code with `git clone
   https://github.com/MotoAcidic/eunod` and checkout the source code of the
...
7. Proceed to verify the tag with `git verify-tag <TAG>` and compile the
   ...
   `EUNODBUILDSYS=OS-ARCH ./release/release.sh <TAG>`.
8. Extract the archive found in the `eunod-<TAG>` directory created by the
   release script and recompute the `SHA256` hash of the release binaries (eunod
   and eunoctl) with `shasum -a 256 <filename>`. These should match __exactly__
