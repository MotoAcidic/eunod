...
func commandUsage(method string) {
	usage, err := eunojson.MethodUsageText(method)
	if err != nil {
...
func main() {
	...
	method := args[0]
	usageFlags, err := eunojson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := eunojson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// eunojson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(eunojson.Error); ok {
		...
		// The error is not a eunojson.Error and this really should not
	...
	marshalledJSON, err := eunojson.MarshalCmd(eunojson.RpcVersion1, 1, cmd)
	if err != nil {
