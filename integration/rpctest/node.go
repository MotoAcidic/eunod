...
import (
	...
	rpc "github.com/MotoAcidic/eunod/rpcclient"
	"github.com/MotoAcidic/eunod/eunoutil"
)

// nodeConfig contains all the args, and data required to launch a eunod process
...
func newConfig(prefix, certFile, keyFile string, extra []string,
	...
	var eunodPath string
	if customExePath != "" {
		eunodPath = customExePath
	} else {
		var err error
		eunodPath, err = eunodExecutablePath()
		if err != nil {
			eunodPath = "eunod"
		}
	}
...
// arguments returns an array of arguments that be used to launch the eunod
// process.
...
// command returns the exec.Cmd which will be used to start the eunod process.
...
// to the eunod process that is launched via Start().
...
// as the base for the log and data directories for eunod.
...
// start creates a new eunod process, and writes its pid in a file reserved for
...
// stop interrupts the running eunod process process, and waits until it exits
...
// shutdown terminates the running eunod process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := eunoutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
