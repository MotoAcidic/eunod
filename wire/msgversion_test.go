...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.EunoDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.EunoDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.EunoDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		err = msg.EunoDecode(buf, test.pver, test.enc)
...
func TestVersionOptionalFields(t *testing.T) {
	...
	uaVersion.UserAgent = "/eunodtest:0.0.1/"
