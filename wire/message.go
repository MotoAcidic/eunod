...
type Message interface {
	EunoDecode(io.Reader, uint32, MessageEncoding) error
	EunoEncode(io.Writer, uint32, MessageEncoding) error
	Command() string
...
func WriteMessageN(w io.Writer, msg Message, pver uint32, eunonet BitcoinNet) (int, error) {
	return WriteMessageWithEncodingN(w, msg, pver, eunonet, BaseEncoding)
}
...
func WriteMessage(w io.Writer, msg Message, pver uint32, eunonet BitcoinNet) error {
	_, err := WriteMessageN(w, msg, pver, eunonet)
	return err
}
...
func WriteMessageWithEncodingN(w io.Writer, msg Message, pver uint32,
	eunonet BitcoinNet, encoding MessageEncoding) (int, error) {
	...
	var bw bytes.Buffer
	err := msg.EunoEncode(&bw, pver, encoding)
	if err != nil {
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, eunonet BitcoinNet,
	...
	// MsgVersion EunoDecode function requires it.
	pr := bytes.NewBuffer(payload)
	err = msg.EunoDecode(pr, pver, enc)
	if err != nil {
		return totalBytes, nil, nil, err
	}
...
func ReadMessageN(r io.Reader, pver uint32, eunonet BitcoinNet) (int, Message, []byte, error) {
	return ReadMessageWithEncodingN(r, pver, eunonet, BaseEncoding)
}
...
func ReadMessage(r io.Reader, pver uint32, eunonet BitcoinNet) (Message, []byte, error) {
	_, msg, buf, err := ReadMessageN(r, pver, eunonet)
	return msg, buf, err
}
