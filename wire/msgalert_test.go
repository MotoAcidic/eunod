...
func TestMsgAlert(t *testing.T) {
	...
	// Test EunoEncode with Payload == nil
	var buf bytes.Buffer
	err := msg.EunoEncode(&buf, pver, encoding)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if !bytes.Equal(buf.Bytes(), expectedBuf) {
		t.Errorf("EunoEncode got: %s want: %s",
...
func TestMsgAlertWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.in.EunoEncode(&buf, test.pver, test.enc)
		if err != nil {
			t.Errorf("EunoEncode #%d error %v", i, err)
			continue
		}
		if !bytes.Equal(buf.Bytes(), test.buf) {
			t.Errorf("EunoEncode #%d\n got: %s want: %s", i,
		...
		err = msg.EunoDecode(rbuf, test.pver, test.enc)
		if err != nil {
			t.Errorf("EunoDecode #%d error %v", i, err)
			continue
		}
		if !reflect.DeepEqual(&msg, test.out) {
			t.Errorf("EunoDecode #%d\n got: %s want: %s", i,
...
func TestMsgAlertWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.EunoEncode(w, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("EunoEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("EunoEncode #%d wrong error got: %v, "+
		...
		err = msg.EunoDecode(r, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.readErr) {
			t.Errorf("EunoDecode #%d wrong error got: %v, want: %v",
				i, err, test.readErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("EunoDecode #%d wrong error got: %v, "+
	...
	w := new(bytes.Buffer)
	err := baseMsgAlert.EunoEncode(w, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.EunoEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
	...
	buf := *new(bytes.Buffer)
	err = baseMsgAlert.EunoEncode(&buf, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.EunoEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
