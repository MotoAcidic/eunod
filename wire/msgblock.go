...
func (msg *MsgBlock) EunoDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if txCount > maxTxPerBlock {
		...
		return messageError("MsgBlock.EunoDecode", str)
	}

	msg.Transactions = make([]*MsgTx, 0, txCount)
	for i := uint64(0); i < txCount; i++ {
		tx := MsgTx{}
		err := tx.EunoDecode(r, pver, enc)
		if err != nil {
			return err
		}
		msg.Transactions = append(msg.Transactions, &tx)
...
// Version field in the block.  This function differs from EunoDecode in that
// EunoDecode decodes from the bitcoin wire protocol as it was sent across the
...
func (msg *MsgBlock) Deserialize(r io.Reader) error {
	...
	//
	// Passing an encoding type of WitnessEncoding to EunoEncode for the
...
func (msg *MsgBlock) EunoEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
...
// This function differs from EunoEncode in that EunoEncode encodes the block to
