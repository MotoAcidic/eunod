...
const (
	...
	MinHighPriority = eunoutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr eunoutil.Address) (*eunoutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *eunoutil.Tx, height int32) error {
...
func logSkippedDeps(tx *eunoutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress eunoutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*eunoutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := eunoutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := eunoutil.NewBlock(&msgBlock)
...
func AddWitnessCommitment(coinbaseTx *eunoutil.Tx,
	blockTxns []*eunoutil.Tx) []byte {
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A eunoutil.Block should use saved in the state to avoid
