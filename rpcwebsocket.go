...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *eunoutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *eunoutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *eunoutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a eunoutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := eunoutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a eunoutil.Address) bool {
	switch a := a.(type) {
	case *eunoutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a eunoutil.Address) {
	switch a := a.(type) {
	case *eunoutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected eunoutil.Block
type notificationBlockDisconnected eunoutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *eunoutil.Tx,
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	block *eunoutil.Block) {

	// Notify interested websocket clients about the connected block.
	ntfn := eunojson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := eunojson.MarshalCmd(eunojson.RpcVersion1, nil, ntfn)
	if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *eunoutil.Block) {
	...
	ntfn := eunojson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := eunojson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := eunojson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *eunoutil.Tx) {
	...
	ntfn := eunojson.NewTxAcceptedNtfn(txHashStr, eunoutil.Amount(amount).ToEUNO())
	marshalledJSON, err := eunojson.MarshalCmd(eunojson.RpcVersion1, nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *eunojson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = eunojson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = eunojson.MarshalCmd(eunojson.RpcVersion1, nil,
				verboseNtfn)
			if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*eunoutil.Tx)
	for _, op := range ops {
...
// blockDetails creates a BlockDetails struct to include in eunows notifications
// from a block and a transaction's block index.
func blockDetails(block *eunoutil.Block, txIndex int) *eunojson.BlockDetails {
	if block == nil {
		return nil
	}
	return &eunojson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *eunoutil.Block) ([]byte, error) {
	// Create and marshal the notification.
	ntfn := eunojson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return eunojson.MarshalCmd(eunojson.RpcVersion1, nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *eunoutil.Tx, block *eunoutil.Block) {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := eunojson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *eunoutil.Tx,
	clients map[chan struct{}]*wsClient) {

	clientsToNotify := m.subscribedClients(tx, clients)

	if len(clientsToNotify) != 0 {
		n := eunojson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := eunojson.MarshalCmd(eunojson.RpcVersion1, nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *eunoutil.Tx, block *eunoutil.Block) {
	...
	for {
		...
		if !batchedRequest {
			...
			if err != nil {
				...
				jsonErr := &eunojson.RPCError{
					Code:    eunojson.ErrRPCParse.Code,
					Message: "Failed to parse request: " + err.Error(),
				}
				reply, err = createMarshalledReply(eunojson.RpcVersion1, nil, nil, jsonErr)
				if err != nil {
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &eunojson.RPCError{
					Code:    eunojson.ErrRPCInvalidRequest.Code,
			...
			switch authCmd, ok := cmd.cmd.(*eunojson.AuthenticateCmd); {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &eunojson.RPCError{
						Code:    eunojson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = eunojson.MarshalResponse(eunojson.RpcVersion2, nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.HelpCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
	...
	if !valid {
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCInvalidParameter,
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*eunojson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &eunojson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.NotifySpentCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.NotifyReceivedCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.StopNotifySpentCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.StopNotifyReceivedCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := eunoutil.DecodeAddress(addr, params)
		if err != nil {
			return &eunojson.RPCError{
				Code: eunojson.ErrRPCInvalidAddressOrKey,
...
func deserializeOutpoints(serializedOuts []eunojson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = eunojson.RPCError{
	Code:    eunojson.ErrRPCDatabase,
	Message: "Reorganize",
}

// rescanBlock rescans all transactions in a single block.  This is a helper
// function for handleRescan.
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *eunoutil.Block) {
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := eunojson.NewRecvTxNtfn(txHex,
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *eunoutil.Block, params *chaincfg.Params) []string {
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.RescanBlocksCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
	...
	discoveredData := make([]eunojson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &eunojson.RPCError{
				Code:    eunojson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, eunojson.RescannedBlock{
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *eunoutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *eunojson.RescanCmd, lookups *rescanKeys, minBlock,
	maxBlock int32, chain *blockchain.BlockChain) (
	*eunoutil.Block, *chainhash.Hash, error) {
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			rpcsLog.Errorf("Error looking up block range: %v", err)
			return nil, nil, &eunojson.RPCError{
				Code:    eunojson.ErrRPCDatabase,
				Message: "Database error: " + err.Error(),
			}
		}
	...
	loopHashList:
		for i := range hashList {
			...
			n := eunojson.NewRescanProgressNtfn(
				hashList[i].String(), blk.Height(),
				blk.MsgBlock().Header.Timestamp.Unix(),
			)
			mn, err := eunojson.MarshalCmd(eunojson.RpcVersion1, nil, n)
			if err != nil {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*eunojson.RescanCmd)
	if !ok {
		return nil, eunojson.ErrRPCInternal
	}
	...
	// Notify websocket client of the finished rescan.  Due to how eunod
	...
	// been sent.
	n := eunojson.NewRescanFinishedNtfn(
		lastBlockHash.String(), lastBlock.Height(),
		lastBlock.MsgBlock().Header.Timestamp.Unix(),
	)
	if mn, err := eunojson.MarshalCmd(eunojson.RpcVersion1, nil, n); err != nil {
