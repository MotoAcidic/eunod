...
Bitcoin RPC server that uses a eunod/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with eunod (https://github.com/MotoAcidic/eunod),
eunowallet (https://github.com/MotoAcidic/eunowallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, eunod and
eunowallet provide a websocket interface that is more efficient than the standard
...
eunod or eunowallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by eunod and
eunowallet only uses a single connection that remains open and allows
...
All notifications provided by eunod require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by eunod (and
eunowallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that eunod intentionally separates the wallet
functionality into a separate process named eunowallet.  This means if you are
connected to the eunod RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, eunowallet provides pass through treatment
...
detected by type asserting the error in a *eunojson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*eunojson.RPCError); ok {
  		switch jerr.Code {
  		case eunojson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - eunodwebsockets
   ...
   block count
 - eunowalletwebsockets
   Connects to a eunowallet RPC server using TLS-secured websockets, registers
