...
type notificationState struct {
	...
	notifySpent        map[eunojson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[eunojson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[eunojson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *eunoutil.Tx, details *eunojson.BlockDetails)
	...
	//
	// Deprecated: Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *eunoutil.Tx, details *eunojson.BlockDetails)
	...
	// result of a rescan request, due to how eunod may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount eunoutil.Amount)

	// OnTxAccepted is invoked when a transaction is accepted into the
	// memory pool.  It will only be invoked if a preceding call to
	// NotifyNewTransactions with the verbose flag set to true has been
	// made to register for the notification and the function is non-nil.
	OnTxAcceptedVerbose func(txDetails *eunojson.TxRawResult)

	// OnEunodConnected is invoked when a wallet connects or disconnects from
	// eunod.
	//
	// This will only be available when client is connected to a wallet
	// server such as eunowallet.
	OnEunodConnected func(connected bool)

	// OnAccountBalance is invoked with account balance updates.
	//
	// This will only be available when speaking to a wallet server
	// such as eunowallet.
	OnAccountBalance func(account string, balance eunoutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case eunojson.BlockConnectedNtfnMethod:
	...
	case eunojson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBlockDisconnected
	case eunojson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case eunojson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRecvTx
	case eunojson.RecvTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case eunojson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case eunojson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case eunojson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case eunojson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case eunojson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case eunojson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnEunodConnected
	case eunojson.EunodConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnEunodConnected == nil {
			return
		}

		connected, err := parseEunodConnectedNtfnParams(ntfn.Params)
		if err != nil {
			log.Warnf("Received invalid eunod connected "+
				"notification: %v", err)
			return
		}

		c.ntfnHandlers.OnEunodConnected(connected)

	// OnAccountBalance
	case eunojson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case eunojson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
//
// NOTE: This is a eunod extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*eunoutil.Tx, error) {
	...
	transactions := make([]*eunoutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = eunoutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*eunoutil.Tx,
	*eunojson.BlockDetails, error) {
	...
	// JSON object.
	var block *eunojson.BlockDetails
	if len(params) > 1 {
	...
	return eunoutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := eunoutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*eunojson.TxRawResult,
	error) {
	...
	var rawTx eunojson.TxRawResult
...
// parseEunodConnectedNtfnParams parses out the connection status of eunod
// and eunowallet from the parameters of a eunodconnected notification.
func parseEunodConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance eunoutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := eunoutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a eunod extension and requires a websocket connection.
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := eunojson.NewNotifyBlocksCmd()
...
func (c *Client) notifySpentInternal(outpoints []eunojson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := eunojson.NewNotifySpentCmd(outpoints)
	return c.SendCmd(cmd)
}

// newOutPointFromWire constructs the eunojson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) eunojson.OutPoint {
	return eunojson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]eunojson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := eunojson.NewNotifySpentCmd(ops)
	return c.SendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := eunojson.NewNotifyNewTransactionsCmd(&verbose)
	return c.SendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := eunojson.NewNotifyReceivedCmd(addresses)
	return c.SendCmd(cmd)
}
...
//
// Deprecated: Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []eunoutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := eunojson.NewNotifyReceivedCmd(addrs)
	return c.SendCmd(cmd)
}
...
//
// Deprecated: Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []eunoutil.Address) error {
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := eunojson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.SendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []eunoutil.Address, outpoints []*wire.OutPoint,
	...
	cmd := eunojson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.SendCmd(cmd)
}
...
// NOTE: This is a eunod extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []eunoutil.Address,
	...
	outPointObjects := make([]eunojson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = eunojson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := eunojson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.SendCmd(cmd)
}
...
// NOTE: This is a eunod extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []eunoutil.Address, outPoints []wire.OutPoint) error {
