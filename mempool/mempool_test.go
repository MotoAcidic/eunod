...
func (s *fakeChain) FetchUtxoView(tx *eunoutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *eunoutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *eunoutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   eunoutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
type poolHarness struct {
	...
	signKey     *eunoec.PrivateKey
	payAddr     eunoutil.Address
	payScript   []byte
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*eunoutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee eunoutil.Amount,
	signalsReplacement bool) (*eunoutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput eunoutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*eunoutil.Tx, error) {
	txChain := make([]*eunoutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, eunoutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	signKey, signPub := eunoec.PrivKeyFromBytes(keyBytes)

	// Generate associated pay-to-script-hash address and resulting payment
	// script.
	pubKeyBytes := signPub.SerializeCompressed()
	payPubKeyAddr, err := eunoutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *eunoutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee eunoutil.Amount,
	signalsReplacement, confirmed bool) *eunoutil.Tx {
...
func testPoolMembership(tc *testContext, tx *eunoutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   eunoutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *eunoutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*eunoutil.Tx, []*eunoutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*eunoutil.Tx, []*eunoutil.Tx) {
				...
				return tx, []*eunoutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*eunoutil.Tx, []*eunoutil.Tx) {
				...
				return tx, []*eunoutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = eunoutil.SatoshiPerBitcoin
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(ctx *testContext) (*eunoutil.Tx, []*eunoutil.Tx) {
				...
				return childReplace, []*eunoutil.Tx{childNoReplace}
			},
			err: "",
		},
	}
