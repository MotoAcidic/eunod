...
const (
	...
	DefaultMinRelayTxFee = eunoutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee eunoutil.Amount) int64 {
	...
	if minFee < 0 || minFee > eunoutil.MaxSatoshi {
		minFee = eunoutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *eunoutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func IsDust(txOut *wire.TxOut, minRelayTxFee eunoutil.Amount) bool {
...
func checkTransactionStandard(tx *eunoutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee eunoutil.Amount,
...
func GetTxVirtualSize(tx *eunoutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
