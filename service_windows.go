...
import (
	...
	"github.com/MotoAcidic/winsvc/eventlog"
	"github.com/MotoAcidic/winsvc/mgr"
	"github.com/MotoAcidic/winsvc/svc"
)

const (
	// svcName is the name of eunod service.
	svcName = "eunodsvc"
...
// logServiceStartOfDay logs information about eunod when the main server has
...
// eunodService houses the main service handler which handles all service
// updates and launching eunodMain.
type eunodService struct{}
...
// long-running eunodMain (which is the real meat of eunod), handles service
// change requests, and notifies the service control manager of changes.
func (s *eunodService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start eunodMain in a separate goroutine so the service can start
...
// installService attempts to install the eunod service.  Typically this should
...
// removeService attempts to uninstall the eunod service.  Typically this should
...
// startService attempts to start the eunod service.
...
func serviceMain() (bool, error) {
	...
	// needed to run eunod on Windows in CI environments like Travis.
	...
	err = svc.Run(svcName, &eunodService{})
	if err != nil {
