...
import (
	...
	"github.com/MotoAcidic/eunod/addrmgr"
	...
	"github.com/MotoAcidic/eunod/mining/cpuminer"
	"github.com/MotoAcidic/eunod/netsync"
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a eunoutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := eunoutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a eunoutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := eunoutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > eunoutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, eunoutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *eunoutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			eunodLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new eunod server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, eunodLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *eunoutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
