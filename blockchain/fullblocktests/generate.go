...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: eunoutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := eunoec.PrivKeyFromBytes([]byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := eunoutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*eunoutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, eunoutil.NewTx(tx))
	}
...
func additionalCoinbase(amount eunoutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee eunoutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee eunoutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee eunoutil.Amount) *wire.MsgTx {
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.EunoEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.EunoEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}
