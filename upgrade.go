...
// oldEunodHomeDir returns the OS specific home directory eunod used prior to
// version 0.3.3.  This has since been replaced with eunoutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldEunodHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "eunod")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".eunod")
	}
...
// location prior to eunod version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to eunod
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldEunodHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "eunod.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "eunod_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "eunod_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to eunod
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		eunodLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old eunod.conf into new location if needed.
		...
		if ohpEmpty {
			...
			eunodLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to eunod as new versions require it.
