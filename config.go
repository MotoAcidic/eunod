...
import (
	...
	"github.com/MotoAcidic/eunod/connmgr"
...
const (
	defaultConfigFilename        = "eunod.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "eunod.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-eunod.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = eunoutil.AppDataDir("eunod", false)
...
// config defines the configuration options for eunod.
//
...
type config struct {
	...
	AgentBlacklist       []string      `long:"agentblacklist" description:"A comma separated list of user-agent substrings which will cause eunod to reject any peers whose user-agent contains any of the blacklisted substrings."`
	AgentWhitelist       []string      `long:"agentwhitelist" description:"A comma separated list of user-agent substrings which will cause eunod to require all peers' user-agents to contain one of the whitelisted substrings. The blacklist is applied before the blacklist, and an empty whitelist will allow all agents that do not fail the blacklist."`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in EUNO/kB to be considered a non-zero fee."`
...
//
// The above results in eunod functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		MinRelayTxFee:        mempool.DefaultMinRelayTxFee.ToEUNO(),
	...
	if cfg.DisableRPC {
		eunodLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = eunoutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]eunoutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := eunoutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		eunodLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-eunod.conf to the given destination path,
...
// eunodDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func eunodDial(addr net.Addr) (net.Conn, error) {
...
// eunodLookup resolves the IP of the given host using the correct DNS lookup
...
func eunodLookup(host string) ([]net.IP, error) {
