...
func TestFilterInsert(t *testing.T) {
	...
	err = f.MsgFilterLoad().EunoEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsert EunoDecode failed: %v\n", err)
		return
	}
...
func TestFilterFPRange(t *testing.T) {
	...
	for _, test := range tests {
		...
		err = f.MsgFilterLoad().EunoEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
		if err != nil {
			t.Errorf("EunoDecode unexpected error: %v\n", err)
			continue
		}
		if !bytes.Equal(got.Bytes(), want) {
...
func TestFilterInsertWithTweak(t *testing.T) {
	...
	err = f.MsgFilterLoad().EunoEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsertWithTweak EunoDecode failed: %v\n", err)
		return
	}
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := eunoutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(eunoutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := eunoutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := eunoutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
