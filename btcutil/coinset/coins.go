...
func (cs *CoinSet) TotalValue() (value eunoutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue eunoutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue eunoutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount eunoutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue eunoutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue eunoutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue eunoutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue eunoutil.Amount, coins []Coin) (Coins, error) {
...
// eunoutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() eunoutil.Amount {
	return eunoutil.Amount(c.txOut().Value)
}
