...
func TestNewTxFromBytes(t *testing.T) {
	...
	tx, err := eunoutil.NewTxFromBytes(testTxBytes)
	if err != nil {
...
func TestTxErrors(t *testing.T) {
	...
	_, err = eunoutil.NewTxFromBytes(shortBytes)
	if err != io.EOF {
...
func TestTxHasWitness(t *testing.T) {
	...
	tx := eunoutil.NewTx(msgTx)

	tx.WitnessHash() // Populate the witness hash cache
	tx.HasWitness()  // Should not fail (see MotoAcidic/eunod#1543)
	...
	for _, msgTxWithoutWitness := range Block100000.Transactions[1:] {
		txWithoutWitness := eunoutil.NewTx(msgTxWithoutWitness)
