...
var (
	...
	// geunooinbaseAux describes additional data that miners should include
	...
	// data.
	geunooinbaseAux = &eunojson.GetBlockTemplateResultAux{
		Flags: hex.EncodeToString(builderScript(txscript.
			NewScriptBuilder().
			AddData([]byte(mining.CoinbaseFlags)))),
	}

	// geunoapabilities describes additional capabilities returned with a
	// block template generated by the getblocktemplate RPC.    It is
	// declared here to avoid the overhead of creating the slice on every
	// invocation for constant data.
	geunoapabilities = []string{"proposal"}
...
var (
	...
	ErrRPCUnimplemented = &eunojson.RPCError{
		Code:    eunojson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &eunojson.RPCError{
		Code:    eunojson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which eunod has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of eunowallet.
...
// not needed.
func internalRPCError(errStr, context string) *eunojson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return eunojson.NewRPCError(eunojson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *eunojson.RPCError {
	return eunojson.NewRPCError(eunojson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *eunojson.RPCError {
	return eunojson.NewRPCError(eunojson.ErrRPCNoTxInfo,
...
// These commands will be implemented in eunowallet.
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.EunoEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount*eunoutil.SatoshiPerBitcoin > eunoutil.MaxSatoshi {
			return nil, &eunojson.RPCError{
				Code:    eunojson.ErrRPCType,
				Message: "Invalid amount",
			}
		}

		// Decode the provided address.
		addr, err := eunoutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &eunojson.RPCError{
		...
		satoshi, err := eunoutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []eunojson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]eunojson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &eunojson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []eunojson.Vout {
	voutList := make([]eunojson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout eunojson.Vout
		vout.N = uint32(i)
		vout.Value = eunoutil.Amount(v.Value).ToEUNO()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*eunojson.TxRawResult, error) {
	...
	txReply := &eunojson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(eunoutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(eunoutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := eunojson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.DecodeScriptCmd)
	...
	p2sh, err := eunoutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := eunojson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(cfg.miningAddrs) == 0 {
		return nil, &eunojson.RPCError{
			Code: eunojson.ErrRPCInternal.Code,
	...
	if !s.cfg.ChainParams.GenerateSupported {
		return nil, &eunojson.RPCError{
			Code: eunojson.ErrRPCDifficulty,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &eunojson.RPCError{
				Code:    eunojson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*eunojson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result eunojson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = eunojson.Bool(peer.Connected())
		...
		addrs := make([]eunojson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr eunojson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &eunojson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := eunoutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := eunojson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if *c.Verbosity == 1 {
		...
		rawTxns := make([]eunojson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &eunojson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		SoftForks: &eunojson.SoftForks{
			Bip9SoftForks: make(map[string]*eunojson.Bip9SoftForkDescription),
		},
	}
	...
	chainInfo.SoftForks.SoftForks = []*eunojson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.SoftForks.Bip9SoftForks[forkName] = &eunojson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := eunojson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a eunojson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*eunojson.GetBlockTemplateResult, error) {
	...
	transactions := make([]eunojson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := eunoutil.NewTx(tx)
		resultTx := eunojson.GetBlockTemplateResultTx{
	...
	reply := eunojson.GetBlockTemplateResult{
	...
	if useCoinbaseValue {
		reply.CoinbaseAux = geunooinbaseAux
...
func handleGetBlockTemplateRequest(s *rpcServer, request *eunojson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCClientInInitialDownload,
...
// chainErrToGBTErrString converts an error returned from eunochain to a string
...
func handleGetBlockTemplateProposal(s *rpcServer, request *eunojson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &eunojson.RPCError{
			Code: eunojson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*eunojson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*eunojson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &eunojson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToEUNO(),
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &eunojson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := eunojson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := eunojson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &eunojson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*eunojson.GetNetworkHashPSCmd)
...
func handleGetNodeAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.GetNodeAddressesCmd)
	...
	addresses := make([]*eunojson.GetNodeAddressesResult, 0, count)
	for _, node := range nodes[:count] {
		address := &eunojson.GetNodeAddressesResult{
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*eunojson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &eunojson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &eunojson.RPCError{
				Code: eunojson.ErrRPCInvalidTxVout,
	...
	txOutReply := &eunojson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         eunoutil.Amount(value).ToEUNO(),
		ScriptPubKey: eunojson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]eunojson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]eunojson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]eunojson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		vinEntry := eunojson.VinPrevOut{
			Txid:     prevOut.Hash.String(),
			Vout:     prevOut.Index,
			Sequence: txIn.Sequence,
			ScriptSig: &eunojson.ScriptSig{
				Asm: disbuf,
				Hex: hex.EncodeToString(txIn.SignatureScript),
			},
		}
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &eunojson.PrevOut{
				Addresses: encodedAddrs,
				Value:     eunoutil.Amount(originTxOut.Value).ToEUNO(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr eunoutil.Address, numToSkip, numRequested uint32) ([]*eunoutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*eunojson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := eunoutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]eunojson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.SendRawTransactionCmd)
	...
	if err != nil {
		...
		if txRuleErr, ok := ruleErr.Err.(mempool.TxRuleError); ok {
			...
			case strings.Contains(
				...
				code = eunojson.ErrRPCTxAlreadyInChain

			default:
				code = eunojson.ErrRPCTxRejected
			}
		}
...
func handleSignMessageWithPrivKey(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.SignMessageWithPrivKeyCmd)

	wif, err := eunoutil.DecodeWIF(c.PrivKey)
	if err != nil {
		message := "Invalid private key"
		switch err {
		case eunoutil.ErrMalformedPrivateKey:
			message = "Malformed private key"
		case eunoutil.ErrChecksumMismatch:
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.ValidateAddressCmd)

	result := eunojson.ValidateAddressChainResult{}
	addr, err := eunoutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
	...
	case *eunoutil.AddressPubKeyHash:
		result.IsScript = eunojson.Bool(false)
		result.IsWitness = eunojson.Bool(false)

	case *eunoutil.AddressScriptHash:
		result.IsScript = eunojson.Bool(true)
		result.IsWitness = eunojson.Bool(false)
	...
	case *eunoutil.AddressWitnessPubKeyHash:
		result.IsScript = eunojson.Bool(false)
		result.IsWitness = eunojson.Bool(true)
		result.WitnessVersion = eunojson.Int32(int32(addr.WitnessVersion()))
		result.WitnessProgram = eunojson.String(hex.EncodeToString(addr.WitnessProgram()))
	...
	default:
		// Handle the case when a new Address is supported by eunoutil, but none
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*eunojson.VerifyMessageCmd)
	...
	if _, ok := addr.(*eunoutil.AddressPubKeyHash); !ok {
		return nil, &eunojson.RPCError{
			Code:    eunojson.ErrRPCType,
	...
	address, err := eunoutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
//
// NOTE: This is a MotoAcidic extension ported from github.com/decred/dcrd.
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]eunojson.VersionResult{
		"eunodjsonrpcapi": {
...
type parsedRPCCmd struct {
	jsonrpc eunojson.RPCVersion
	id      interface{}
	method  string
	cmd     interface{}
	err     *eunojson.RPCError
}
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, eunojson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *eunojson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(eunojson.Error); ok &&
			jerr.ErrorCode == eunojson.ErrUnregisteredMethod {

			parsedCmd.err = eunojson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = eunojson.NewRPCError(
			eunojson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *eunojson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion eunojson.RPCVersion, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *eunojson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*eunojson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return eunojson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *eunojson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
	...
	if jsonErr == nil {
		...
		if parsedCmd.err != nil {
			...
			result, err = s.standardCmdResult(parsedCmd,
				closeChan)
			if err != nil {
				if rpcErr, ok := err.(*eunojson.RPCError); ok {
					jsonErr = rpcErr
				} else {
					jsonErr = &eunojson.RPCError{
						Code:    eunojson.ErrRPCInvalidRequest.Code,
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = eunojson.MarshalResponse(eunojson.RpcVersion1, nil, nil, jsonErr)
			if err != nil {
		...
		if err == nil {
			...
			//
			// Eunod does not respond to any request without and "id" or "id":null,
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = eunojson.MarshalResponse(eunojson.RpcVersion2, nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = eunojson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="eunod RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *eunoutil.Block, flags blockchain.BehaviorFlags) (bool, error)
